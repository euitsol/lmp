...
class VoyagerUserController extends VoyagerBaseController
{
    public function profile(Request $request)
    {
        ...
        if (!$dataType && app('VoyagerGuard') == 'web') {
            ...
            //direct bypass to profile edit
            // return redirect()->route('voyager.users.edit', Auth::user()->getKey());
        } elseif ($dataType) {
            $route = route('voyager.'.$dataType->slug.'.edit', Auth::user()->getKey());

            //direct bypass to profile edit
            // return redirect()->route('voyager.'.$dataType->slug.'.edit', Auth::user()->getKey());
        }
